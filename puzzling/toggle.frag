...
float noise(float x) {
    ...
    return mix(random(i), random(i + 1.0f), u);
}
...
float resScale(float x) {
    return x * u_px_per_square / 64.0f;
}
...
void main() {
    ...
    vec3 paneColor = hsluvToRgb(vec3(u_hue, 75.0f, l));

    // Render inner fancyness

    float ringMid = 0.45f;
    float ringWidth = mix(0.735f, 0.9f, u_activity);
    float displacementMax = resScale(0.075f);
    float noiseZoom = 18.0f;
    float ringNoise = noise(normalize(sq) * noiseZoom + 10.0f * u_time + u_center_of_square * vec2(351.5f, 531.1f));
    ringMid += mix(-displacementMax, displacementMax, ringNoise);

    float normalizedDist = 1.0f - (abs(ringMid - clamp(length(sq), ringMid - 0.5f * ringWidth, ringMid + 0.5f * ringWidth)) / (ringWidth / 2.0f));
    float intensity = normalizedDist * normalizedDist * 1.0f;
    intensity *= mix(1.2f, 4.0f, u_activity);

    float innerAlpha = smoothstep(0.0f, 0.5f, intensity);
    vec3 innerColor = hsluvToRgb(vec3(u_hue, 75.0f, intensity * 100.0f));

    vec3 combinedColor = paneColor + innerColor;

    outColor = vec4(combinedColor, max(innerAlpha, min(mix(0.4f, 1.0f, u_activity), outerBox)));
    ...
    float lightFactor = mix(mix(25.0f, 6.0f, innerAlpha), 1.0f, easeOutQuadratic(avatarDistNormalized));
